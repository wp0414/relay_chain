...
// Package channel enables access to a channel on a Fabric network. A channel client instance provides a listener to interact with peers on specified channel.
...
//
// A channel client instance provides a listener to interact with peers on specified channel.
...
// InvokeHandler invokes listener using request and optional request options provided
//  Parameters:
...
func (cc *Client) InvokeHandler(handler invoke.Handler, request Request, options ...RequestOption) (Response, error) {
	...
	//Prepare context objects for listener
...
//createReqContext creates req context for invoke listener
