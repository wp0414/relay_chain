...
//NewQueryHandler returns query listener with chain of ProposalProcessorHandler, EndorsementHandler, EndorsementValidationHandler and SignatureValidationHandler
...
//NewExecuteHandler returns execute listener with chain of SelectAndEndorseHandler, EndorsementValidationHandler, SignatureValidationHandler and CommitHandler
...
//NewProposalProcessorHandler returns a listener that selects proposal processors
func NewProposalProcessorHandler(next ...Handler) *ProposalProcessorHandler {
	return &ProposalProcessorHandler{next: getNext(next)}
}

//NewEndorsementHandler returns a listener that endorses a transaction proposal
func NewEndorsementHandler(next ...Handler) *EndorsementHandler {
	return &EndorsementHandler{next: getNext(next)}
}

//NewEndorsementHandlerWithOpts returns a listener that endorses a transaction proposal
...
//NewEndorsementValidationHandler returns a listener that validates an endorsement
...
//NewCommitHandler returns a listener that commits transaction propsal responses
