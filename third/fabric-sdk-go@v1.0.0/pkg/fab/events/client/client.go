...
// SetAfterConnectHandler registers a listener that is called
...
// SetBeforeReconnectHandler registers a listener that will be called
...
func (c *Client) t(handlerImp handler, errch chan error) error {
	if err1 := handlerImp(); err1 != nil {
		logger.Warnf("Error invoking afterConnect listener: %s. Disconnecting...", err1)
		...
		return errors.WithMessage(err1, "error invoking afterConnect listener")
	}
	return nil
}
...
func (c *Client) reconnect() {
	...
	if handlerImp != nil {
		if err := handlerImp(); err != nil {
			logger.Errorf("Error invoking beforeReconnect listener: %s", err)
			return
		}
	}
