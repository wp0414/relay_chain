...
// Handler is the listener for a given event type.
...
// RegisterHandler registers an event listener
func (ed *Dispatcher) RegisterHandler(t interface{}, h Handler) {
	htype := reflect.TypeOf(t)
	if _, ok := ed.handlers[htype]; !ok {
		logger.Debugf("Registering listener for %s on dispatcher %T", htype, ed)
		ed.handlers[htype] = h
	} else {
		logger.Debugf("Cannot register listener %s on dispatcher %T since it's already registered", htype, ed)
	}
}
