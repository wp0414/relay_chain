...
func TestDiscoveryAccessDenied(t *testing.T) {
	...
	// Subsequent calls should succeed since the error listener should have replaced the discovery service
	...
	// Calls should still succeed since the error listener ignores transient errors
	_, err = discoveryService.GetPeers()
	require.NoError(t, err)
}
