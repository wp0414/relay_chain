...
func NewWithData(initializer InitializerWithData, opts ...options.Opt) *Reference {
	...
	if lazyRef.expirationProvider != nil {
		...
		// called, set a timer that will call the expiration listener.
...
func (r *Reference) handleExpiration() {
	...
	logger.Debug("Invoking expiration listener")
	r.expirationHandler()
}

// resetValue is an expiration listener that calls the
...
// refreshValue is an expiration listener that calls the
